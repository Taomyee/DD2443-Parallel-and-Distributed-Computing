...
public class Experiment {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            ArrayList<Log.Entry[]> logs = lockFreeSet.getLog();
            //integrate logs according to timestamps
            Log.Entry[] log = Log.integrate(logs);
            System.out.println("log size:" + log.length);

            /*for (Log.Entry entry : log) {
                System.out.println(entry.toString());
            }*/
            // Check sequential consistency
            System.out.println(Log.validate(lockFreeSet, log));
        } catch (Exception e) {
            e.printStackTrace();
            System.exit(1);
        }
